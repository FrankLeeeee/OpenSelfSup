...
class ContrastiveODC_V24_ABL_STUDY_4(nn.Module):
    ...
    def __init__(
        ...
        super(ContrastiveODC_V24_ABL_STUDY_4, self).__init__()
    ...
    def get_features(self, labels):
        ...
        pos_feature_idx_list = [
            torch.zeros_like(pos_feature_idx) for i in range(self.world_size)
        ]
        neg_feature_idx_list = [
            torch.zeros_like(neg_feature_idx) for i in range(self.world_size)
        ]
        ...
        pos_feature_collection = torch.zeros(
            size=(labels.size(0) * self.world_size,
        ...
        # get the features
        if self.rank == 0:
            pos_feature_collection = self.memory_bank.feature_bank[
                pos_feature_idx_list][:].cuda()
            neg_feature_collection = self.memory_bank.feature_bank[
                neg_feature_idx_list][:].cuda()
        ...
        bs = labels.size(0)
        pos_feature = pos_feature_collection[self.rank * bs:(self.rank + 1) *
                                             bs]
        neg_feature = neg_feature_collection[self.rank * bs *
                                             self.num_neg_features:(self.rank +
                                                                    1) * bs *
                                             self.num_neg_features]
    ...
    def _get_feature_idx(self, labels):
        ...
        for label in labels:
            ...
            pos_feature_idx = (self.memory_bank.label_bank == label).nonzero(
            ...
            for cluster_idx in close_cluster_idx_list:
                neg_feature_idx = (
                    self.memory_bank.label_bank == cluster_idx).nonzero(
                        as_tuple=False).flatten().tolist()
                neg_feature_idx_all.extend(neg_feature_idx)
            neg_feature_idx_all = random.sample(neg_feature_idx_all,
                                                self.num_neg_features)
    ...
    def _get_close_cluster(self):
        dis = torch.cdist(self.memory_bank.centroids,
    ...
    def get_updated_features(self, idx):
        ...
        idx_collection = torch.zeros(
            size=(bs * self.world_size, self.memory_bank.feat_dim)).cuda()
    ...
    def forward_train(self, img, idx, **kwargs):
        ...
        feature_to_odc = [pair[0] for pair in feature_pairs]
        feature_to_cts = [pair[1] for pair in feature_pairs]
        feature_to_odc = torch.stack(feature_to_odc, dim=0)
        feature_to_cts = torch.stack(feature_to_cts, dim=0)

        score_to_odc = self.head([feature_to_odc])
        score_to_cts = self.head([feature_to_cts])
        ...
        cls_pos = torch.mul(feat_after_update,
                            pos_features).sum(dim=1).unsqueeze(1)
        cls_neg = torch.mul(feat_after_update.unsqueeze(1),
                            neg_features).sum(dim=2)

        # loss input
        loss_inputs = dict()
        loss_inputs['instance_positive'] = ins_pos
        loss_inputs['instance_negative'] = ins_neg
        loss_inputs['outs_to_odc'] = score_to_odc
        loss_inputs['outs_to_cts'] = score_to_cts
