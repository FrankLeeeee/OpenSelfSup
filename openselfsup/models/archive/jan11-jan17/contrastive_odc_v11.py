...
class ContrastiveODC_V11(nn.Module):
    ...
    def __init__(self,
                 ...
                 neg_num=64,
                 neg_select=32):
        super(ContrastiveODC_V11, self).__init__()
    ...
    def get_centroids(self, cls_labels):
        return self.memory_bank.centroids[cls_labels]
    ...
    def forward_train(self, img, idx, **kwargs):
        ...
        # centroid_features = self.get_centroids(cls_labels)
        ...
        # ins_pos
        ins_pos = torch.mul(z, z_prime).sum(dim=1).unsqueeze(1)

        # print(z.shape)
        # print(centroid_features.shape)
        # print(cluster_negative_feature.shape)
        # cluster_pos = torch.mul(z, centroid_features).sum(dim=1).unsqueeze(1)
        # print(cluster_pos.shape)

        cluster_neg = torch.mul(
            z.unsqueeze(1),
            cluster_negative_feature).sum(dim=2)
        cluster_neg = cluster_neg.sort(dim=1)[0][:, :self.neg_select]
        # print(cluster_negative_feature.shape)
        ...
        loss_inputs['cluster_positive'] = ins_pos
