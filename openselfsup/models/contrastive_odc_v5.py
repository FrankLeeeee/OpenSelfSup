...
def get_features(num, output, close_centroids, length, label_bank, feature_bank, feat_dim, i):
    ...
    index = torch.cat(index_list, dim=1).squeeze(1)
...
class ContrastiveODC_V5(nn.Module):
    ...
    def __init__(self,
                 ...
                 train_bs=32,
                 neg_num=32):
        super(ContrastiveODC_V5, self).__init__()
    ...
    def forward_train(self, img, idx, **kwargs):
        ...
        # NOTE instance-level contrastive loss
        # torch.cuda.synchronize()
        # cts_start = time.time()
        # assert z.size(0) % 2 == 0
        # N = z.size(0) // 2
        # s = torch.matmul(z, z.permute(1, 0))  # (2N)x(2N)
        # mask, pos_ind, ins_neg_mask = self._create_buffer(N)
        # # remove diagonal, (2N)x(2N-1)
        # s = torch.masked_select(s, mask == 1).reshape(s.size(0), -1)
        # # get positive
        # ins_positive = s[pos_ind].unsqueeze(1)  # (2N)x1
        # # select negative, (2N)x(2N-2)
        # ins_negative = torch.masked_select(
        #     s, ins_neg_mask == 1).reshape(s.size(0), -1)

        ###
        # only keep those saved into feature bank
        ###
        # positive = positive[::2]
        ...
        # negative = torch.matmul(z[::2], negative_feature.permute(1, 0))
        # negative = torch.zeros((z.size(0) // 2, self.neg_num)).cuda()

        # for i in range(N):
        #     negative[i] = torch.matmul(z[i*2],
        ...
        # loss input
        loss_inputs = dict()
        # loss_inputs['positive'] = ins_positive
        # loss_inputs['negative'] = ins_negative
        loss_inputs['positive'] = None
        loss_inputs['negative'] = None
