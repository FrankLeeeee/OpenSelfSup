...
class ContrastiveODCHead_V15(nn.Module):
    ...
    def __init__(self,
        ...
        self.instance_contrastive_criterion = nn.CrossEntropyLoss()
        self.cluster_contrastive_criterion = nn.CrossEntropyLoss()
    ...
    def calc_instance_cts_loss(self, pos, neg):
        ...
        loss = self.instance_contrastive_criterion(logits, labels)
        return loss
    ...
    def calc_cluster_cts_loss(self, pos, neg):
        ...
        logits /= self.temperature
        labels = torch.zeros((N, ), dtype=torch.long).cuda()
        loss = self.cluster_contrastive_criterion(logits, labels)
        return loss
    ...
    def loss(self,
        ...
        ins_cts_loss = self.calc_instance_cts_loss(
            instance_positive, instance_negative)
        cluster_cts_loss = self.calc_cluster_cts_loss(
            cluster_positive, cluster_negative)
        losses = dict()
        losses['cls_loss'] = cls_loss
        losses['ins_cts_loss'] = ins_cts_loss
        losses['cluster_cts_loss'] = cluster_cts_loss
        losses['acc'] = acc
        losses['loss'] = cls_loss + ins_cts_loss + cluster_cts_loss
