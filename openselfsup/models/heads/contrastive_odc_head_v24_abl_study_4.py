...
class ContrastiveODCHead_V24_ABL_STUDY_4(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_classes=1000,
                 temperature=0.1):
        super(ContrastiveODCHead_V24_ABL_STUDY_4, self).__init__()
        self.alpha = alpha
        self.beta = beta
    ...
    def calc_cls_score_ctc_loss(self, outs_to_odc, outs_to_cts):
        return self.cls_score_contrastive_criterion(outs_to_odc[0],
                                                    outs_to_cts[0])
    ...
    def loss(
        ...
        ins_cts_loss = self.calc_ins_cts_loss(instance_positive,
                                              instance_negative)
        cls_cts_loss = self.calc_cls_cts_loss(cluster_positive,
                                              cluster_negative)
        cls_cts_loss = self.calc_cls_score_ctc_loss(outs_to_odc, outs_to_cts)
        losses = dict()
        losses['ins_cts_loss'] = ins_cts_loss
        losses['cls_cts_loss'] = cls_cts_loss
        losses['cls_cts_loss'] = cls_cts_loss
        losses['loss'] = ins_cts_loss + cls_cts_loss + cls_cts_loss
