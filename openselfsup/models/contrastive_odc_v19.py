...
class ContrastiveODC_V19(nn.Module):
    ...
    def __init__(self,
        ...
        super(ContrastiveODC_V19, self).__init__()
    ...
    def forward_train(self, img, idx, **kwargs):
        ...
        bs = idx.size(0)
        neg_features = neg_features.view(bs*self.num_neg_features, -1)
        combine = torch.cat([
            feature_to_odc, feature_to_cts, pos_features, neg_features
            ], dim=0)
        ...
        pos_features_norm = combine[bs*2:bs*3]
        neg_features_norm = combine[bs*3:].view(bs, self.num_neg_features, -1)
        
        pos_1 = torch.mul(feature_to_odc_norm, pos_features_norm).sum(dim=1).unsqueeze(1)
        pos_2 = torch.mul(feature_to_cts_norm, pos_features_norm).sum(dim=1).unsqueeze(1)
        neg_1 = torch.mul(feature_to_odc_norm.unsqueeze(1), neg_features_norm).sum(dim=2)
        neg_2 = torch.mul(feature_to_cts_norm.unsqueeze(1), neg_features_norm).sum(dim=2)
