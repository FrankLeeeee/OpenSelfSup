...
def get_random_index(label_bank, label):
    return random.sample((label_bank == label).nonzero().flatten().tolist(),
                         1)[0]


@MODELS.register_module
class ContrastiveODC_V24_ABL_STUDY_5(nn.Module):
    ...
    def __init__(
        ...
        super(ContrastiveODC_V24_ABL_STUDY_5, self).__init__()
    ...
    def get_random_feature_from_each_cluster(self):
        ...
        if self.rank == 0:
            fn = partial(get_random_index, self.memory_bank.label_bank)
            index = self.pool.map(fn, range(self.memory_bank.num_classes))
            feature_collect = self.memory_bank.feature_bank[index][:].cuda()

        torch.distributed.broadcast(feature_collect, src=0)
    ...
    def forward_train(self, img, idx, **kwargs):
        ...
        # for updating in the memory bank
        ...
        # classification loss
        feature_norm = feature[0] / (
            feature[0].norm(dim=1).view(-1, 1) + 1e-10)
        feature_norm_pairs = torch.split(
            feature_norm, split_size_or_sections=2, dim=0)

        random_features = self.get_random_feature_from_each_cluster()

        outs_norm_odd = [pair[0] for pair in feature_norm_pairs]
        outs_norm_even = [pair[1] for pair in feature_norm_pairs]
        outs_norm_odd = torch.stack(outs_norm_odd, dim=0)
        outs_norm_even = torch.stack(outs_norm_even, dim=0)
        outs_norm_odd_cls_score = torch.matmul(outs_norm_odd,
                                               random_features.permute(1, 0))
        outs_norm_even_cls_score = torch.matmul(outs_norm_even,
                                                random_features.permute(1, 0))
        ...
        loss_inputs['outs_to_odc'] = outs_norm_odd_cls_score
