...
def get_features(num, output, close_centroids, length, label_bank, feature_bank, feat_dim, i):
    ...
    for j in range(len(index_list)-1, -1, -1):
        count += index_list[j].size(0)

        if count > num or j == 0:
            index = torch.cat(index_list[j:], dim=0).squeeze(1)
...
class ContrastiveODC_V14_2(nn.Module):
    ...
    def __init__(self,
                 ...
                 pool_size=32,
                 neg_num=32):
        super(ContrastiveODC_V14_2, self).__init__()
    ...
    def get_old_features(self, idx):
        ...
        if self.rank == 0:
            feature_collection = self.memory_bank.feature_bank[idx_list].cuda()

        torch.distributed.broadcast(feature_collection, src=0)
        old_features = feature_collection[self.rank * bs: (self.rank+1) * bs]
        return old_features
    ...
    def forward_train(self, img, idx, **kwargs):
        ...
        cluster_pos = torch.mul(
            z, cluster_pos_features).sum(dim=1).unsqueeze(1)
        # print(cluster_pos.shape)

        cluster_neg = torch.mul(
            z.unsqueeze(1),
            cluster_negative_feature).sum(dim=2)
        # print(cluster_negative_feature.shape)
        ...
        loss_inputs['cluster_positive'] = cluster_pos
        loss_inputs['cluster_negative'] = cluster_neg
